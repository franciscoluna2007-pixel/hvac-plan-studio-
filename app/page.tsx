...
const tools = [
  ...
  { id: "rangeHood", label: "Range Hood", icon: Fan },
  { id: "dryerVent", label: "Dryer Vent", icon: AirVent },
...
const branchNetworkKinds = ["supply", "return"] as const satisfies readonly DirectBranchNetworkKind[];
function isBranchNetworkKind(type: string): type is DirectBranchNetworkKind {
  return branchNetworkKinds.includes(type as DirectBranchNetworkKind);
}
function branchNetworkName(kind: DirectBranchNetworkKind) {
  return kind === "return" ? "return" : "supply";
}
function branchNetworkColor(kind: DirectBranchNetworkKind) {
  return kind === "return" ? "red" : "blue";
}
type SymbolKind = "diffuser" | "returnGrille" | "equipment" | "fan" | DedicatedExhaustSymbolKind | "damper" | "motorDamper" | "reducer" | "thermostat" | "smoke" | "airflow" | "note";
const symbolTools: SymbolKind[] = ["diffuser", "returnGrille", "equipment", "fan", "rangeHood", "dryerVent", "damper", "motorDamper", "reducer", "thermostat", "smoke", "airflow", "note"];
type SymbolPreset = {
  id: string;
...
function symbolFamily(preset: SymbolPreset) {
  ...
  if (isDedicatedExhaustSymbolKind(preset.kind)) return "Exhaust";
...
function SymbolArtwork({ kind, variant = "", width = 24, height = 24 }: { kind: SymbolKind; variant?: string; width?: number; height?: number }) {
  ...
  if (kind === "rangeHood") return <>
    <path className="exhaust-device-body range-hood-body" d="M -18 10 L 18 10 L 12 -8 L -12 -8 Z" />
    <rect className="exhaust-device-collar" x="-5" y="-15" width="10" height="7" rx="1.5" />
    <path className="exhaust-device-detail" d="M -13 5 L 13 5 M -10 0 L 10 0 M -7 -5 L 7 -5" />
    <path className="exhaust-device-flow" d="M 0 -15 L 0 -24 M -4 -20 L 0 -24 L 4 -20" />
  </>;
  if (kind === "dryerVent") return <>
    <circle className="exhaust-device-body dryer-vent-body" cx="0" cy="0" r="14" />
    <circle className="exhaust-device-collar" cx="0" cy="0" r="7" />
    <path className="exhaust-device-detail dryer-vent-vane" d="M -7 -5 Q 0 -1 7 -5 M -7 0 Q 0 4 7 0 M -7 5 Q 0 9 7 5" />
    <path className="exhaust-device-flow" d="M 14 0 L 24 0 M 20 -4 L 24 0 L 20 4" />
  </>;
...
type BranchPreview = {
  ...
  style?: "wye45" | "tee90";
  parentSize: string;
  portSizes: [string, string, string];
  valid: boolean;
...
type EditTransactionSnapshot = {
  ...
  port3BranchDraft: Port3BranchDraftState | null;
...
function HVACPlanStudioApp() {
  ...
  const [planContextMenu, setPlanContextMenu] = useState<{
    drawingId: string;
    x: number;
    y: number;
  } | null>(null);
  ...
  function showPlanPage(nextPage: number) {
    ...
    if (port3BranchDraft && !port3BranchResolvedRef.current) finishDrawing();
  ...
  function activatePlanTool(tool: string) {
    ...
    if (port3BranchDraft && tool !== (port3BranchDraft.networkKind || "supply")) {
      if (!port3BranchResolvedRef.current) finishDrawing();
    }
    setActiveTool(tool);
  }
  ...
  const port3BranchResolvedRef = useRef(true);
  ...
  useEffect(() => {
    if (
      activeTool === "branch" ||
      (port3BranchDraft && activeTool === (port3BranchDraft.networkKind || "supply"))
    ) return;
    setPendingBranchFittingId(null);
    setPort3BranchDraft(null);
    setBranchPreview(null);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      if (activeTool === "branch") {
        setBranchMessage("Ready for the next fitting · click the supply or return trunk where it belongs");
      }
    }, 4500);
    return () => window.clearTimeout(timer);
  ...
  function fittingBranchNetworkKind(
    fitting: Drawing | undefined,
    sourceDrawings = drawings,
  ): DirectBranchNetworkKind | null {
    if (!fitting?.fitting) return null;
    for (const connectedId of fitting.fitting.connectedIds) {
      const connected = sourceDrawings.find((drawing) => drawing.id === connectedId);
      if (connected && isBranchNetworkKind(connected.type)) return connected.type;
    }
    return null;
  }
  ...
  function drawingLayer(drawing: Drawing): LayerId | null {
    ...
    if (isDedicatedExhaustSymbolKind(drawing.symbol?.kind)) return "branch";
  ...
  function releasePlanPointerCapture(pointerId: number) {
    [planOverlayRef.current, canvasViewportRef.current].forEach((target) => {
      if (!target) return;
      try {
  ...
  function capturePlanPointer(target: Element, pointerId: number) {
    try {
      if (!target.hasPointerCapture(pointerId)) {
  ...
  function restoreEditTransaction(pointerId: number) {
    ...
    if (snapshot?.pointerId === pointerId) {
      ...
      setPort3BranchDraft(snapshot.port3BranchDraft);
      port3BranchResolvedRef.current = !snapshot.port3BranchDraft;
  ...
  function handleViewportPointerDownCapture(event: PointerEvent<HTMLDivElement>) {
    ...
    const planEditControl = event.target instanceof Element
      ...
      : null;
    const planDrawingControl = event.target instanceof Element
      ? event.target.closest("[data-plan-drawing-id]")
      : null;
    const planDrawingId = planDrawingControl?.getAttribute("data-plan-drawing-id");
    ...
    if (event.pointerType === "touch") {
      if (!pdf) return;
      const directTouchPointer = directTouchPointerRef.current;
      if (
        directTouchPointer &&
        directTouchPointer.pointerId !== event.pointerId
      ) {
        event.preventDefault();
        event.stopPropagation();
        directTouchPointerRef.current = null;
        cancelPlanPointerInteraction(directTouchPointer.pointerId);
        touchPointersRef.current.set(
          directTouchPointer.pointerId,
          directTouchPointer.point,
        );
      ...
      if (directTouchEdit) {
        ...
        capturePlanPointer(
          planOverlayRef.current || event.currentTarget,
          event.pointerId,
        );
        directTouchPointerRef.current = {
          pointerId: event.pointerId,
  ...
  function handleViewportPointerMoveCapture(event: PointerEvent<HTMLDivElement>) {
    if (event.pointerType === "touch") {
      if (directTouchPointerRef.current?.pointerId === event.pointerId) {
        directTouchPointerRef.current.point = {
          x: event.clientX,
          y: event.clientY,
        };
  ...
  function nearestBranchSegment(point: Point, onlyDrawingId?: string) {
  ...
  function resolveDeterministicDirectBranchTarget(
  ...
  ): ReturnType<typeof nearestBranchSegment> {
    const candidate = resolveDirectBranchTrunkCandidate({
      ...
      zoom,
      inputType,
      networkKinds: branchNetworkKinds,
    });
  ...
  function orientMainTowardAirflow<T extends NonNullable<ReturnType<typeof nearestBranchSegment>>>(target: T) {
  ...
  function queuedBranchRoute(center: Point, mainId: string, mainAngle: number, branchRunId = queuedBranchRunId) {
    const main = drawings.find((candidate) => candidate.id === mainId);
    const networkKind = main && isBranchNetworkKind(main.type) ? main.type : null;
    const drawing = drawings.find((candidate) =>
      networkKind &&
      candidate.id === branchRunId &&
      candidate.id !== mainId &&
      candidate.page === pageNumber &&
      candidate.type === networkKind &&
      !candidate.fitting &&
      !candidate.symbol &&
  ...
  function branchOpportunities(): BranchOpportunity[] {
    const branchRuns = drawings.filter((drawing) =>
      drawing.page === pageNumber &&
      isBranchNetworkKind(drawing.type) &&
      !drawing.fitting &&
      !drawing.symbol &&
  ...
  function focusNextBranchOpportunity(opportunities = branchOpportunities()) {
    if (!opportunities.length) {
      setBranchMessage("No obvious unconnected junctions found · you can still click a supply or return trunk manually");
      return;
    }
    ...
    setBranchPreview({
      ...
      portSizes: [
        ...
        steppedSize(opportunity.parentSize, 1),
        drawings.find((drawing) => drawing.id === opportunity.branchRunId)?.size
          || steppedSize(opportunity.parentSize, 2),
      ],
      valid: true,
  ...
  function nearestAttachableBranchSegment(point: Point, fittingId: string, onlyDrawingId?: string) {
  ...
  function attachPendingBranchRun(point: Point, onlyDrawingId?: string) {
    ...
    const candidate = nearestAttachableBranchSegment(point, pendingBranchFittingId, onlyDrawingId);
    if (!fitting?.fitting || !candidate || candidate.distance > BRANCH_ATTACH_RADIUS_PX / zoom) {
      const networkKind = fittingBranchNetworkKind(fitting);
      setBranchMessage(networkKind
        ? `Click directly on the ${branchNetworkColor(networkKind)} ${branchNetworkName(networkKind)} run you want connected to the open branch port`
        : "Reconnect a supply or return trunk before attaching Port 3");
      return true;
    }
    if (!branchLeavesTrunkAtClearAngle(
      fitting.fitting.angle,
      candidate.angle,
    )) {
      setBranchMessage("Choose a branch run that leaves the trunk at a clear angle");
      return true;
    }
    const resolvedStyle = branchStyle === "auto"
  ...
  function existingThreeRunJunction(point: Point, clickedRunId: string): ThreeRunBranchMatch | null {
    ...
    if (
      !clickedRun ||
      !isBranchNetworkKind(clickedRun.type) ||
  ...
  function fittingPortVisual(fitting: Drawing, port: 0 | 1 | 2) {
    ...
    const connectedRun = drawings.find((drawing) =>
      ...
      !drawing.fitting &&
      !drawing.symbol &&
      (!networkKind || drawing.type === networkKind)
    );
    const fallbackSize = [
  ...
  function reattachFittingIn(drawingsToRepair: Drawing[], fittingId: string) {
    ...
    const networkKind = fittingBranchNetworkKind(fitting, drawingsToRepair);
    if (!networkKind) return { drawings: drawingsToRepair, connected: 0 };
  ...
  function reattachSelectedFitting() {
    ...
    if (!selected) return;
    if (port3BranchDraft?.fittingId === selected.id) {
      setBranchMessage("Finish or keep the Port 3 branch open before repairing this fitting");
      return;
    }
  ...
  function branchNetworkTrace(fitting?: Drawing) {
    ...
    const adjacency = networkKind === "return" ? network.returnAdjacency : network.children;
    const pending = rootRunId ? [rootRunId] : [];
    while (pending.length) {
      const runId = pending.shift()!;
      if (runIds.has(runId)) continue;
      runIds.add(runId);
      (adjacency.get(runId) || []).forEach((childId) => pending.push(childId));
    }

    drawings
      .filter((drawing) => drawing.fitting && fittingBranchNetworkKind(drawing) === networkKind && (
        networkKind === "return"
          ? drawing.fitting.connectedIds.some((runId) => runIds.has(runId))
  ...
  function branchNetworkConnectionHealth(fitting?: Drawing) {
    ...
    for (const fittingId of trace.fittingIds) {
      ...
      const networkKind = fittingBranchNetworkKind(networkFitting);
      if (!networkKind) continue;
  ...
  function buildFieldConnectionModel(systemId: string) {
    ...
    fittings.forEach((fitting) => {
      ...
      fitting.fitting!.connectedIds.forEach((runId, port) => {
        ...
        if (!run || run.page !== fitting.page || run.type !== networkKind) return;
  ...
  function beginPort3BranchDraft(
    ...
    if (!fitting.fitting || drawingLocked(fitting)) {
      setBranchMessage("Unlock the branch layer before drawing the Port 3 branch");
      return;
    }
    const networkKind = fittingBranchNetworkKind(fitting);
    if (!networkKind) {
      setBranchMessage("Reconnect a supply or return trunk before drawing the Port 3 branch");
      return;
    }
    const branchPort = fittingPortPoints(fitting)[2];
    const branchSize = fitting.fitting.branchSize;
    const systemId = drawingSystem(fitting);
    port3BranchResolvedRef.current = false;
    setPendingBranchFittingId(null);
    setPort3BranchDraft({
      fittingId: fitting.id,
      branchSize,
    ...
    setDraft([branchPort]);
    setHoverPoint(branchPort);
    activatePlanTool(networkKind);
    setBranchPlacementResult(null);
    setBranchMessage(
      `Port 3 ready · draw the ${branchSize}" branch and right-click to finish`,
    );
  }

  function leavePort3BranchOpen() {
    const fittingId = port3BranchDraft?.fittingId || pendingBranchFittingId;
    port3BranchResolvedRef.current = true;
    setPort3BranchDraft(null);
    setPendingBranchFittingId(null);
    setContinuingRunId(null);
    ...
    setBranchPreview(null);
    activatePlanTool("branch");
    if (fittingId) setSelectedId(fittingId);
    setBranchMessage("Fitting kept with Port 3 open · select it later to finish or attach a run");
  }

  function attachExistingPort3RunInstead(fittingIdOverride?: string) {
    const fittingId = port3BranchDraft?.fittingId || fittingIdOverride;
    if (!fittingId) return;
    port3BranchResolvedRef.current = true;
    const fitting = drawings.find((drawing) =>
      drawing.id === fittingId &&
      drawing.fitting &&
      drawing.page === pageNumber
    );
    setPort3BranchDraft(null);
    ...
    if (!fitting?.fitting) {
      setPendingBranchFittingId(null);
      setPort3BranchDraft(null);
      setBranchMessage("That fitting is no longer available");
      return;
    }
    setSelectedId(fitting.id);
    ...
    const networkKind = fittingBranchNetworkKind(fitting);
    setBranchMessage(networkKind
      ? `Attach existing run · click the ${branchNetworkColor(networkKind)} ${branchNetworkName(networkKind)} run for Port 3`
      : "Attach existing run · reconnect a supply or return trunk first");
  }

  function resolveDirectBranchPlacement(
    target: NonNullable<ReturnType<typeof nearestBranchSegment>> & {
      points: Point[];
      reversed: boolean;
    },
    intentPoint?: Point,
  ...
  function previewDirectBranchPlacementGesture(
    ...
    if (pendingBranchFittingId) {
      ...
      const candidate = fitting
        ? nearestAttachableBranchSegment(point, pendingBranchFittingId, lockedDrawingId)
        : null;
      if (!fitting?.fitting) {
        directBranchPlacementGestureRef.current = null;
        setPendingBranchFittingId(null);
      ...
      setBranchMessage(candidateReady
        ...
        : gesture
          ? "Keep the pen on the selected branch run, then release"
          : "Press directly on the matching supply or return branch run you want to attach");
      return;
    }
    ...
    if (!rawTarget) {
      ...
      setBranchMessage(gesture
        ...
        : "Press directly on one unlocked supply or return run in the active system");
      return;
    }
  ...
  function beginDirectBranchPlacementGesture(
    ...
    setBranchPlacementResult(null);
    if (pendingBranchFittingId) {
      const candidate = nearestAttachableBranchSegment(point, pendingBranchFittingId);
      if (!candidate || candidate.distance > BRANCH_ATTACH_RADIUS_PX / zoom) {
        setBranchMessage("Press directly on the matching supply or return branch run you want to attach");
        return;
      }
    ...
    if (!target) {
      setBranchPreview(null);
      setExplicitThreeRunOption(null);
      setBranchMessage("Press directly on one unlocked supply or return run in the active system");
      return;
    }
    const gesture: DirectBranchPlacementGesture = {
      pointerId: event.pointerId,
  ...
  function placeSmartBranch(
    point: Point,
    lockedTarget?: NonNullable<ReturnType<typeof nearestBranchSegment>>,
    intentPoint?: Point,
  ) {
    setBranchPlacementResult(null);
    ...
    const branchRunId = matchedRoute?.drawing.id || "";
    ...
    setBranchPreview(null);
    setExplicitThreeRunOption(null);
    const completionMessage = matchedRoute
      ? "T Branch placed - selected trunk split - existing branch connected"
      : "T Branch placed - selected trunk split - connect or draw Port 3";
    setBranchMessage(completionMessage);
    if (matchedRoute) {
      setPendingBranchFittingId(null);
      activatePlanTool("select");
  ...
  function updateFittingPortSize(port: 0 | 1 | 2, size: string) {
    ...
    if (port3BranchDraft?.fittingId === fitting.id) {
      setBranchMessage("Finish or keep the Port 3 branch open before changing fitting sizes");
      return;
    }
  ...
  function assignSelectedFittingPort(port: 0 | 1 | 2, runId: string) {
    ...
    if (!fitting?.fitting) return;
    const networkKind = fittingBranchNetworkKind(fitting);
    const run = drawings.find((drawing) => drawing.id === runId && drawing.type === networkKind && !drawing.fitting);
    if (!networkKind || !run) return;
    if (port3BranchDraft?.fittingId === fitting.id) {
      setBranchMessage("Finish or keep the Port 3 branch open before reconnecting fitting ports");
      return;
    }
  ...
  function fittingPortState(fitting: Drawing, port: 0 | 1 | 2) {
    ...
    const recommended = recommendedDuctSize(cfm, run.type === "return" ? "return" : "supply");
  ...
  function reshapeSelectedFitting(nextStyle: "wye45" | "tee90", nextSide?: 1 | -1) {
    ...
    if (!fitting?.fitting) return;
    if (port3BranchDraft?.fittingId === fitting.id) {
      setBranchMessage("Finish or keep the Port 3 branch open before changing fitting geometry");
      return;
    }
  ...
  function placeSymbol(kind: SymbolKind, point: Point) {
    const defaults: Record<SymbolKind, { label: string; size: string; cfm: number }> = {
      ...
      rangeHood: { label: "Range Hood", size: "FIELD VERIFY", cfm: 0 },
      dryerVent: { label: "Dryer Vent", size: "FIELD VERIFY", cfm: 0 },
  ...
  function finishDrawing() {
    if (port3BranchDraft) {
      if (port3BranchResolvedRef.current) return;
      port3BranchResolvedRef.current = true;
      const fitting = drawings.find((drawing) =>
        drawing.id === port3BranchDraft.fittingId &&
        drawing.fitting &&
        drawing.page === port3BranchDraft.page &&
        drawingSystem(drawing) === port3BranchDraft.systemId
      );
      const branchPoints = cleanPoints(draft);
      const port3IsOpen = !fitting?.fitting?.connectedIds[2];
      const currentBranchPort = fitting?.fitting
        ? fittingPortPoints(fitting)[2]
        : port3BranchDraft.anchor;
      const livePort3Draft = {
        ...port3BranchDraft,
        anchor: currentBranchPort,
        branchSize: fitting?.fitting?.branchSize || port3BranchDraft.branchSize,
      };
      const networkKind = port3BranchDraft.networkKind || fittingBranchNetworkKind(fitting) || "supply";
      const routeStartsAtCurrentPort = Boolean(
        branchPoints[0] &&
        Math.hypot(
          branchPoints[0].x - currentBranchPort.x,
          branchPoints[0].y - currentBranchPort.y,
        ) <= 0.5
      );
      if (
        fitting?.fitting &&
        port3IsOpen &&
        !drawingLocked(fitting) &&
        activeTool === networkKind &&
        routeStartsAtCurrentPort &&
        ...
        const drawing: Drawing = {
          ...
          type: networkKind,
          points: branchPoints,
          size: livePort3Draft.branchSize,
          lineWeight: runLineWeights[networkKind],
          page: port3BranchDraft.page,
          cfm: 0,
          cfmSource: "planning-seed",
          systemId: port3BranchDraft.systemId,
          elevation: fitting.elevation || "",
          sizeReviewed: false,
        };
        const connected = addJunctionPoints(drawings, [branchPoints[branchPoints.length - 1]]);
        const commit = commitPort3Branch({
          drawings: connected,
          draft: livePort3Draft,
          run: drawing,
        });
        if (commit.ok) {
          setHistory(
            linkRunToMatchingEquipmentPlenum(
              commit.drawings,
              drawingId,
            ),
          );
          const completionMessage = `${fitting.fitting.style === "tee90" ? "90° tee" : "45° wye"} complete · Port 3 branch drawn and connected`;
          setSelectedId(fitting.id);
          setBranchMessage(completionMessage);
          setBranchPlacementResult({
            fittingId: fitting.id,
            message: completionMessage,
          });
          setActiveTool("branch");
        } else {
          setBranchMessage("The Port 3 branch was not saved because its route no longer starts at the fitting");
        }
      } else if (branchPoints.length > 1) {
        setBranchMessage("The Port 3 branch was not saved because its fitting is unavailable or locked");
      } else {
        setBranchMessage("Fitting kept with Port 3 open · no branch stub was saved");
        setActiveTool("branch");
      }
      setPort3BranchDraft(null);
      setPendingBranchFittingId(null);
  ...
  function continueFittingOutlet(port: 1 | 2) {
    ...
    if (!fitting?.fitting) return;
    if (port3BranchDraft?.fittingId === fitting.id) {
      setBranchMessage("Finish or keep the Port 3 branch open before extending another outlet");
      return;
    }
    const run = drawings.find((drawing) => drawing.id === fitting.fitting!.connectedIds[port]);
    if (!run) return;
  ...
  function undo() {
    ...
    if (undoDirectPlacement) {
      ...
      setBranchHoverRunId(null);
      setActiveTool("branch");
      setBranchMessage("T Branch placement undone · click the supply or return trunk to place it again");
    }
  ...
  function redo() {
    if (draft.length || port3BranchDraft) {
      setBranchMessage("Finish or keep the current route open before redoing another change");
      return;
    }
  ...
  function clearDeletedDrawingState(idsToDelete: string[]) {
    ...
    if (port3BranchDraft && deleted.has(port3BranchDraft.fittingId)) {
      port3BranchResolvedRef.current = true;
      setPort3BranchDraft(null);
      setDraft([]);
    }
  ...
  function deleteSelected() {
    if (!selectedId) return;
    const deletionSelection = selectedIds.length ? selectedIds : [selectedId];
    if (
      port3BranchDraft &&
      connectedSelection(deletionSelection).includes(port3BranchDraft.fittingId)
    ) {
      setBranchMessage("Finish or keep the Port 3 branch open before deleting this fitting network");
      return;
    }
  ...
  function removeFittingAndHeal(fitting: Drawing) {
    if (!fitting.fitting) return;
    if (port3BranchDraft?.fittingId === fitting.id) {
      setBranchMessage("Finish or keep the Port 3 branch open before removing this fitting");
      return;
    }
  ...
  function selectedPlanCopyTemplate(): PlanCopyTemplate<Drawing> | null {
    ...
    const routeSelection = [selected, ...selectedDrawings].some((drawing) =>
      Boolean(drawing && (isBranchNetworkKind(drawing.type) || drawing.fitting)));
    if (routeSelection) {
      if (selectedDrawings.some(drawingLocked)) {
        setBranchMessage("Unlock this HVAC layer before copying the duct assembly");
        return null;
      }
      ...
      if (!template) {
        setBranchMessage("Select a connected supply or return run, or a T Branch, before copying the assembly");
      }
      return template;
    }
    if (selectedIds.length > 1) {
      setBranchMessage("Select one standalone icon or measurement, or select a connected duct assembly");
      return null;
    }
  ...
  function armCopyPlacement(
    ...
    setBranchMessage(template.version === 2
      ? "Duct assembly follows your mouse - click to place repeatedly - Esc or right-click finishes"
  ...
  function copySelected() {
    ...
    setBranchMessage(template.version === 2
      ? "Duct assembly copied - press Ctrl+V or choose Copy & place"
  ...
  function pasteDrawing() {
    ...
    if (!template) {
      setBranchMessage("Select and copy an icon, measurement, or connected duct assembly first");
      return;
    }
  ...
  function nudgeSelection(dx: number, dy: number) {
    ...
    if (
      port3BranchDraft &&
      connectedSelection(selectedIds).includes(port3BranchDraft.fittingId)
    ) {
  ...
  function startFittingDrag(event: PointerEvent<SVGGElement>, drawing: Drawing) {
    ...
    if (port3BranchDraft?.fittingId === drawing.id) {
      event.stopPropagation();
      setBranchMessage("Finish or keep the Port 3 branch open before moving this fitting");
      return;
    }
    event.stopPropagation();
    if (event.shiftKey) {
  ...
  function symbolLabelBaseY(drawing: Drawing) {
    ...
    return kind === "equipment"
      ...
      : kind === "fan"
        ? -31
        : isDedicatedExhaustSymbolKind(kind)
          ? kind === "rangeHood" ? -30 : -27
        : kind === "airflow"
          ? -12
          : ["diffuser", "returnGrille"].includes(kind)
            ? -symbolHeight / 2 - 10
  ...
  function symbolResizeBounds(drawing: Drawing) {
    ...
    if (drawing.symbol?.kind === "rangeHood") return { width: 40, height: 36 };
    if (drawing.symbol?.kind === "dryerVent") return { width: 36, height: 36 };
    return {
  ...
  function startGroupDrag(event: PointerEvent<SVGElement>, drawingId: string) {
    const ids = connectedSelection(selectedIds);
    if (port3BranchDraft && ids.includes(port3BranchDraft.fittingId)) {
      event.stopPropagation();
      setBranchMessage("Finish or keep the Port 3 branch open before moving this fitting network");
      return;
    }
  ...
  const selectedPort3FittingLocked = Boolean(
    port3BranchDraft &&
    selectedDrawing?.id === port3BranchDraft.fittingId
  );
  const selectedDrawingLocked = Boolean(
    selectedDrawing &&
    (drawingLocked(selectedDrawing) || selectedPort3FittingLocked)
  );
  ...
  const projectCommands: ProjectCommand[] = [
    ...
    },
    {
      ...
      detail: "Click the supply or return trunk exactly where the fitting belongs",
      group: "Draw",
      shortcut: "B",
    ...
    <main data-layout="command-deck" data-visual-world="material-traverse" data-presentation="material-cobalt" className={`app-shell swiss-plan-workspace field-first-workspace layout-${workspaceLayout} density-${workspaceDensity} render-${renderQuality} ${workspaceLayout !== "desktop" ? "tablet-layout" : ""} ${fieldMode ? "field-mode" : ""} ${fieldRedline.open ? "redline-open" : ""} ${leftPanelOpen ? "" : "left-closed"} ${rightPanelOpen ? "" : "right-closed"} ${showCloudProjects ? "cloud-open" : ""} ${showProjectHome ? "project-home-open" : ""} ${showPlanIntelligence ? "plan-intelligence-open" : ""} ${showFieldPackageComposer ? "field-package-open" : ""} ${showFinishJobStudio ? "finish-job-open" : ""} ${showSystemBalanceStudio ? "system-balance-open" : ""} ${showMarkupAssistant ? "markup-assistant-open" : ""} ${["rooms", "checks"].includes(rightTab) && rightPanelOpen ? "wide-inspector" : ""} ${packagePrintClasses} ${packagePrintReleased ? "package-print-released" : "package-print-draft"}`}>
      ...
      <section className="workspace" inert={modalWorkspaceActive ? true : undefined} aria-hidden={modalWorkspaceActive}>
        ...
        <aside id="workspace-tools-panel" className={`left-panel view-${leftPanelView}`} aria-label="HVAC plan tools">
            ...
            <div className={`branch-designer ${activeTool === "branch" ? "active" : ""}`}>
              ...
              {activeTool !== "branch" && <>
                <div className="branch-safe-mode" role="status">
                  ...
                  <span>
                    <b>Select the trunk</b>
                    <small>Port 3 stays open until you connect or draw a branch.</small>
                  </span>
                </div>
                <button className="branch-arm" onClick={() => {
                  ...
                  setBranchPreview(null);
                  setBranchPlacementResult(null);
                  setBranchMessage("Place on the supply or return trunk");
                }}>
              ...
              {port3BranchDraft && <div className="branch-link-step">
                ...
                <button onClick={() => attachExistingPort3RunInstead()}>Attach an existing run instead</button>
                <button onClick={leavePort3BranchOpen}>Keep Port 3 open</button>
              </div>}
              {pendingBranchFittingId && <div className="branch-link-step">
                <button onClick={() => {
                  const fitting = drawings.find((drawing) => drawing.id === pendingBranchFittingId);
                  if (fitting) beginPort3BranchDraft(fitting, "direct-placement");
                }}>Draw from Port 3</button>
                  ...
                  <div className="symbol-catalog-grid" role="list" aria-label={`${symbolCategory} symbol catalog`}>
                    {visiblePresets.map((item) => {
                      ...
                      return <button
                        ...
                        aria-label={`${item.label} · ${item.size}`}
          ...
          <div className="panel-section">
            ...
            {selectedDrawing?.symbol ? <>
              ...
              {selectedPort3FittingLocked && <div className="port3-edit-lock" role="status">
                <strong>PORT 3 BRANCH IN PROGRESS</strong>
                <span>Finish the route, keep the port open, or attach an existing run before editing this fitting.</span>
              </div>}
              ...
              <div className="fitting-actions">
                ...
                {!selectedDrawing.fitting.connectedIds[2] && <button
                  className="reattach-action"
                  onClick={() => beginPort3BranchDraft(selectedDrawing)}
                >Draw Port 3 branch</button>}
                <button className="reattach-action" onClick={() => {
                  const fitting = drawings.find((drawing) => drawing.id === selectedId && drawing.fitting);
                  if (!fitting) return;
                  attachExistingPort3RunInstead(fitting.id);
              ...
              {(["Upstream / inlet", "Outlet A / straight", "Outlet B / branch"] as const).map((label, port) => {
                const selectedFittingDrawing = drawings.find((drawing) => drawing.id === selectedId)!;
                const fitting = selectedFittingDrawing.fitting!;
                const networkKind = fittingBranchNetworkKind(selectedFittingDrawing);
        ...
        <section className="canvas-area">
          ...
          >
            {planContextMenu && !copyPlacement && (() => {
              const contextDrawing = drawings.find((drawing) => drawing.id === planContextMenu.drawingId);
              const copiesAssembly = Boolean(
                contextDrawing && (isBranchNetworkKind(contextDrawing.type) || contextDrawing.fitting),
              );
              return contextDrawing ? <div
                ref={planContextMenuRef}
                className="plan-context-menu"
                role="menu"
                aria-label={copiesAssembly ? "Duct assembly actions" : "Selected plan item actions"}
                data-canvas-ui
              ...
              >
                <span>{copiesAssembly ? "DUCT ASSEMBLY" : contextDrawing.symbol ? "HVAC SYMBOL" : "PLAN ITEM"}</span>
                <button
                  type="button"
                  role="menuitem"
                  onClick={() => {
            ...
            {draft.length > 0 && ["supply", "return", "fresh"].includes(activeTool) && <div className="live-draft-hud" data-canvas-ui>
              <span>{port3BranchDraft ? "PORT 3 BRANCH" : "LIVE RUN"}</span>
              <strong>{ductSize}&quot; · {liveDraftFeet.toFixed(1)} LF</strong>
              <b>{liveDraftCfm} CFM · {liveDraftVelocity} FPM</b>
              <small>{port3BranchDraft
                ? "Draw from the fitting · right-click finishes and connects Port 3"
                : "Left-click direction · Shift locks angle · Right-click finishes"}</small>
              {port3BranchDraft && <div className="branch-workflow-actions">
                <button type="button" onClick={() => attachExistingPort3RunInstead()}>Attach existing run instead</button>
                <button type="button" onClick={leavePort3BranchOpen}>Keep Port 3 open</button>
              </div>}
            </div>}
            {pdf && activeTool === "branch" && <div className="visually-hidden" aria-live="polite">
              {branchPlacementResult?.message || branchMessage || "Place on the supply or return trunk"}
            </div>}
                  ...
                  >
                    ...
                    {[...drawings, ...(copyPlacement?.preview || [])].filter((drawing) => {
                      ...
                      if (drawing.fitting) {
                        ...
                        const showPortGuides =
                          pendingBranchFittingId === drawing.id ||
                          port3BranchDraft?.fittingId === drawing.id;
                        const fittingChromeScale = fittingOverlayScale(zoom);
                        ...
                        >
                          <path
                            ...
                            d={`M ${inlet.x} ${inlet.y} L ${center.x} ${center.y} L ${outlet.x} ${outlet.y}`}
                            vectorEffect="non-scaling-stroke"
                            style={{ strokeWidth: FITTING_HIT_STROKE_PX }}
                          />
                          <path
                            className="fitting-hit"
                            d={`M ${center.x} ${center.y} L ${branchPort.x} ${branchPort.y}`}
                            vectorEffect="non-scaling-stroke"
                            style={{ strokeWidth: FITTING_HIT_STROKE_PX }}
                          />
                          ...
                          {showFittingLabels && <text
                            ...
                            y="0"
                            textAnchor="middle"
                            transform={`translate(${fittingLabelPoint.x} ${fittingLabelPoint.y}) scale(${fittingChromeScale})`}
                          >{drawing.fitting.style === "tee90" ? "TEE" : "WYE"} {portSizes.join("×")}{drawing.elevation ? ` · EL ${drawing.elevation}` : ""}</text>
                          }
                          {branchPlacementResult?.fittingId === drawing.id && <text
                            className="connection-confirmed-label"
                            x="0"
                            y="0"
                            textAnchor="middle"
                            transform={`translate(${center.x} ${center.y - 30 * fittingChromeScale}) scale(${fittingChromeScale})`}
                          >✓ 3 / 3 CONNECTED</text>}
                        </g>;
                      }
                      const isCopyPreview = drawing.id.startsWith("copy-place-preview-");
                      ...
                      return <g key={drawing.id} data-plan-edit-control={isCopyPreview ? undefined : "hvac"} data-plan-drawing-id={isCopyPreview ? undefined : drawing.id} tabIndex={isCopyPreview ? undefined : 0} role={isCopyPreview ? undefined : "button"} aria-label={isCopyPreview ? undefined : planDrawingAccessibleLabel(drawing)} aria-pressed={isCopyPreview ? undefined : runSelected} aria-disabled={isCopyPreview ? undefined : drawingLocked(drawing)} aria-hidden={isCopyPreview || undefined} style={isCopyPreview ? { pointerEvents: "none" } : undefined} className={`${activeTrace.runIds.has(drawing.id) ? "traced-run" : ""} ${runSelected ? "selected-drawing" : ""} ${branchCandidateClass} ${assistantPreviewClass} ${isCopyPreview ? "copy-place-preview" : ""}`.trim()} onFocus={isCopyPreview ? undefined : () => selectOnly(drawing.id)} onKeyDown={isCopyPreview ? undefined : (event) => handlePlanDrawingKeyDown(event, drawing)} onPointerDown={isCopyPreview ? undefined : (event) => {
                        ...
                        <path className="hit-line" d={path} onPointerDown={isCopyPreview ? undefined : (event) => startLineDrag(event, drawing)}>
                          <title>Drag this duct run to move it. Connected fittings repair when you release.</title>
                        </path>
                    ...
                    {activeTool === "branch" && pendingBranchFittingId && !branchPreview && (() => {
                      const fitting = drawings.find((drawing) => drawing.id === pendingBranchFittingId && drawing.fitting);
                      if (!fitting?.fitting) return null;
                      const center = fitting.points[0];
                      const cueScale = fittingOverlayScale(zoom);
                      return <g className="branch-connection-cue" transform={`translate(${center.x + 14 * cueScale} ${center.y - 18 * cueScale}) scale(${cueScale})`}>
                        <rect x="0" y="-18" width="116" height="22" rx="4" />
                        <text x="9" y="-4">Connect branch</text>
                      </g>;
                    })()}
                    {branchPreview && (() => {
                      const center = branchPreview.center;
                      const previewStyle = branchPreview.style || "wye45";
                      const previewScale = fittingOverlayScale(zoom);
                      const previewFitting: Drawing = {
                        id: "branch-port-preview-render",
                        type: "branch",
                        points: [center],
                        size: branchPreview.portSizes.join("×"),
                        page: pageNumber,
                        systemId: activeSystem,
                        fitting: {
                          kind: "ty",
                          ...
                          side: branchPreview.side,
                          upstreamSize: branchPreview.portSizes[0],
                          downstreamSize: branchPreview.portSizes[1],
                          branchSize: branchPreview.portSizes[2],
                          connectedIds: [],
                        },
                      };
                      ...
                      const placementInstruction = !branchPreview.valid
                        ? "Move along trunk"
                        : branchPreview.mode === "attach-run"
                          ...
                          : branchPreview.matchedExisting
                            ? "Release to connect"
                            : "Release to place fitting";
                      const instructionWidth = Math.max(92, placementInstruction.length * 5.7 + 18);
                      return <g className={`branch-preview ${branchPreview.valid ? "" : "invalid"}`}>
                        ...
                        {[inlet, outlet, branchPort].map((port, index) => <circle
                          ...
                          className={`preview-port ${
                            !branchPreview.valid
                              ? "missing"
                              : index === 2 && !branchPreview.matchedExisting
                                ? "open"
                                : "ready"
                          }`}
                          cx={port.x}
                        ...
                        />)}
                        <g
                          className="branch-preview-instruction"
                          transform={`translate(${branchPort.x + 9 * previewScale} ${branchPort.y - 10 * previewScale}) scale(${previewScale})`}
                        >
                          <rect x="0" y="-18" width={instructionWidth} height="22" rx="4" />
                          <text x="9" y="-4">{placementInstruction}</text>
                        </g>
                      </g>;
                    })()}
                    {symbolPreview && (() => {
                      ...
                      const fallback: Record<SymbolKind, { label: string; size: string; cfm: number; elevation: string }> = {
                        ...
                        rangeHood: { label: "Range Hood", size: "FIELD VERIFY", cfm: 0, elevation: "" },
                        dryerVent: { label: "Dryer Vent", size: "FIELD VERIFY", cfm: 0, elevation: "" },
      ...
      {mountFieldRedlineStudio && fieldRedline.activeLayer && <FieldRedlineStudio
        ...
        eraserSize={fieldRedline.eraserSize}
        layer={fieldRedline.activeLayer}
        ...
        onEraserSizeChange={fieldRedline.setEraserSize}
      ...
      <footer inert={modalWorkspaceActive ? true : undefined} aria-hidden={modalWorkspaceActive}>
        ...
        {activeTool !== "branch" && <span>{selectedIds.length ? `${selectedIds.length} selected · Arrow nudge · Shift+Arrow 10× · midpoint grips stretch` : "Right-click drag pans anywhere · left-click selects/draws · wheel zooms at cursor · two-finger touch navigates · stylus draws"}</span>}
